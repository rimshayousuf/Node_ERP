const db = require("../../models-Clients/index");
const ResponseLog = require("../../../core/ResponseLog");
const SeqFunc = require("../../../core/SeqFunc");
const JV = require("../../../core/GenerateJV");
const GetCodes = require("../../../core/GetCodes");

exports.postData = async (req, res) => {
    try {
        let TransNo = req.body.Header.TransNo;


        let Header = await req.dbconn.FIN_Purchases.findOne({ where: { TransNo } })
        let DetailData = await req.dbconn.FIN_PurchaseDetail.findAll({ where: { TransNo } })
        let Detail = []

        let VenAccount = await GetCodes.AccountCodes(req, 'Vendors', Header.VendCode)

        if (Header.TransType === 'PI') {
            //Discount    
            if (Header.Discount > 0) {
                Detail.push({
                    AcctCode: VenAccount.Account.DiscAcctCode,
                    AcctDesc: VenAccount.Account.DiscAcctDesc,
                    AcctType: 'Discount',
                    LineDr: 0,
                    LineCr: Header.Discount,
                    LineDrCur: 0,
                    LineCrCur: Header.DiscountCur,
                    JobCode: '',
                    JobDesc: '',
                    TaxCode: '',
                    TaxDesc: '',
                    LineDesc: '',
                    TaxDr: 0.0,
                    TaxCr: 0.0,
                    TaxDrCur: 0.0,
                    TaxCrCur: 0.0,
                    TaxLine: 0,
                    Recon: 0,
                    ReconDate: null,
                    LoanID: null,
                })
            }
            //Freight
            if (Header.Freight > 0) {
                Detail.push({
                    AcctCode: VenAccount.Account.FrgtAcctCode,
                    AcctDesc: VenAccount.Account.FrgtAcctDesc,
                    AcctType: 'Freight',
                    LineDr: Header.Freight,
                    LineCr: 0,
                    LineDrCur: Header.FreightCur,
                    LineCrCur: 0,
                    JobCode: '',
                    JobDesc: '',
                    TaxCode: '',
                    TaxDesc: '',
                    LineDesc: '',
                    TaxDr: 0.0,
                    TaxCr: 0.0,
                    TaxDrCur: 0.0,
                    TaxCrCur: 0.0,
                    TaxLine: 0,
                    Recon: 0,
                    ReconDate: null,
                    LoanID: null,
                })
            }
        }


        DetailData = DetailData.filter(v => v.AcctCode !== '')
        await Promise.all(
            DetailData.map(async v => {

                let Pay =
                {
                    AcctCode: v.AcctCode,
                    AcctDesc: v.AcctDesc,
                    AcctType: 'Purchases',
                    LineDr: Header.TransType === 'PDI' ? 0 : v.Amount,
                    LineCr: Header.TransType === 'PDI' ? v.Amount : 0,
                    LineDrCur: Header.TransType === 'PDI' ? 0 : v.Amount_Cur,
                    LineCrCur: Header.TransType === 'PDI' ? v.Amount_Cur : 0,
                    JobCode: '',
                    JobDesc: '',
                    TaxCode: '',
                    TaxDesc: '',
                    LineDesc: '',
                    TaxDr: 0.0,
                    TaxCr: 0.0,
                    TaxDrCur: 0.0,
                    TaxCrCur: 0.0,
                    TaxLine: 0,
                    Recon: 0,
                    ReconDate: null,
                    LoanID: null,

                }
                Detail.push(Pay)

                let Tax = await GetCodes.AccountCodes(req, 'Taxes', v.TaxDetailCode)

                if (v.TaxAmount > 0) {

                    let Item = {
                        AcctCode: Tax.Account.AcctCode,
                        AcctDesc: Tax.Account.AcctDesc,
                        AcctType: 'Taxes',
                        LineDr: Header.TransType === 'PDI' ? 0 : v.TaxAmount,
                        LineCr: Header.TransType === 'PDI' ? v.TaxAmount : 0,
                        LineDrCur: Header.TransType === 'PDI' ? 0 : v.TaxAmount_Cur,
                        LineCrCur: Header.TransType === 'PDI' ? v.TaxAmount_Cur : 0,
                        JobCode: '',
                        JobDesc: '',
                        TaxCode: '',
                        TaxDesc: '',
                        LineDesc: '',
                        TaxDr: 0.0,
                        TaxCr: 0.0,
                        TaxDrCur: 0.0,
                        TaxCrCur: 0.0,
                        TaxLine: 0,
                        Recon: 0,
                        ReconDate: null,
                        LoanID: null,
                    }
                    Detail.push(Item)

                }
            })
        );

        Detail.push({
            AcctCode: VenAccount.Account.PayAcctCode,
            AcctDesc: VenAccount.Account.PayAcctDesc,
            AcctType: 'Payable',
            LineDr: Header.TransType === 'PDI' ? Header.TotalAmount : 0,
            LineCr: Header.TransType === 'PDI' ? 0 : Header.TotalAmount,
            LineDrCur: Header.TransType === 'PDI' ? Header.TotalAmount : 0,
            LineCrCur: Header.TransType === 'PDI' ? 0 : Header.TotalAmountCur,
            JobCode: Header.JobCode,
            JobDesc: Header.JobDesc,
            TaxCode: '',
            TaxDesc: '',
            LineDesc: '',
            TaxDr: 0.0,
            TaxCr: 0.0,
            TaxDrCur: 0.0,
            TaxCrCur: 0.0,
            TaxLine: 0,
            Recon: 0,
            ReconDate: null,
            LoanID: null,
        })

        let JVData = await JV.Create(req, { Header: { Data: Header }, Detail })

        if (JVData.success) {
            await req.dbconn.FIN_Purchases.update({
                Status: 1,
                PostedUser: req.headers.username,
                postedAt: new Date(),
                JrnlID: JVData.JrnlID
            },
                {
                    where: { TransNo },
                })
            ResponseLog.Send200(req, res, "Record Posted Successfully");
        } else {

            ResponseLog.Error200(req, res, "No Record Found!");
        }
    } catch (err) {
        console.log({ err })
        ResponseLog.Error200(req, res, err.message);
    }
};
