...
exports.getOne = async (req, res) => {
  try {
    let INV = await SeqFunc.getOne(req.dbconn.SOP_InvoiceMaster, {
      where: { TransNo: req.query.TransNo },
    });

    if (INV.success) {
      let Detail = await req.dbconn.SOP_InvoiceDetail.findAll({
        where: { TransNo: req.query.TransNo },
        attributes: [
          "TransNo",
          "ILineSeq",
...
exports.delete = async (req, res) => {
  try {
    ...
    if (INV.success) {
      await SeqFunc.Delete(req.dbconn.SOP_InvoiceTaxes, {
        where: { TransNo: INV.Data.TransNo },
      });
      await SeqFunc.Delete(req.dbconn.SOP_InvoiceDetail, {
        where: { TransNo: INV.Data.TransNo },
      });
      await SeqFunc.Delete(req.dbconn.SOP_InvoiceMaster, {
        where: { TransNo: INV.Data.TransNo },
      });
