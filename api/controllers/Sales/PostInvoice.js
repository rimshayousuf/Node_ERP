...
exports.postData = async (req, res) => {
  try {
    ...
    if (REQ.success) {
      ...
      let INV_Detail = await SeqFunc.getAll(req.dbconn.SOP_InvoiceDetail, { where: { TransNo: TransNo } });
      let Detail = [];

      await Promise.all(
        INV_Detail?.Data?.map(async v => {
          let INV_Taxes = await SeqFunc.getAll(req.dbconn.SOP_InvoiceTaxes, { where: { TransNo: TransNo, ItemCode: v.ItemCode, ILineSeq: v.ILineSeq } });
          await Promise.all(
      ...
      Detail.push({
        ...
        AcctType: 'Sales',
        LineDr: 0,
        LineCr: Header.TransTotal,
        LineDrCur: 0,
        LineCrCur: Header.TransTotal_Cur,
        JobCode: '',
        JobDesc: '',
        TaxCode: '',
        TaxDesc: '',
      ...
      if (JVData.success) {
        await req.dbconn.SOP_InvoiceMaster.update({
          Status: 1,
          Posted: 1,
