...
const Post = require("./PostInvoice");
...
exports.getOne = async (req, res) => {
  try {
    let Invoice = await SeqFunc.getOne(req.dbconn.POP_InvoiceMaster, {
      where: { TransNo: req.query.TransNo },
    });

    if (Invoice.success) {
      let Detail = await req.dbconn.POP_InvoiceDetail.findAll({
        where: { TransNo: req.query.TransNo },
        attributes: [
          "TransNo",
          "ILineSeq",
      ...
      await Promise.all(
        Detail.map(async (val) => {
          let GRData = await req.dbconn.POP_GoodsReceiptDetail.findAll({ attributes: [ [req.dbconn.sequelize.literal('BaseQuantity - (ISNULL(QtyUsed,0) + ISNULL(QtyCanceled,0))'), "AvailableQuantity"] ], where: { TransNo: val.GTransNo, GLineSeq: val.GLineSeq } })
          GRData = JSON.stringify(GRData)
          GRData = JSON.parse(GRData)
          val.AvailableQuantity = GRData[0]?.AvailableQuantity 
          val.Taxes = Taxes.Data.filter((o) => o.ILineSeq === val.ILineSeq && o.TransNo == val.TransNo);
          val.Batches = Batches.Data.filter((o) => o.ILineSeq === val.ILineSeq);
          return val;
        })
      )
...
exports.delete = async (req, res) => {
  try {
    ...
    if (Invoice.success) {
      await SeqFunc.Delete(req.dbconn.POP_InvoiceBatches, {
        where: { TransNo: Invoice.Data.TransNo },
      });

      await SeqFunc.Delete(req.dbconn.POP_InvoiceTaxes, {
        where: { TransNo: Invoice.Data.TransNo },
      });
      await SeqFunc.Delete(req.dbconn.POP_InvoiceDetail, {
        where: { TransNo: Invoice.Data.TransNo },
      });
      await SeqFunc.Delete(req.dbconn.POP_InvoiceMaster, {
        where: { TransNo: Invoice.Data.TransNo },
      });
  ...
  try {
    ...
    if (InvoiceData.success) {
      ...
      Detail.map((o) => {
        ...
        o.GRPrice = o.GRPrice ? o.GRPrice : null;
        o.ILineSeq = LineSeq;
        o.Taxes?.map((l) => {
          delete l.RID;
          l.TransNo = o.TransNo;
